...
import { z } from "zod";
import { createBookingRequest, createCreatorApplication, createCreatorOnboarding, createGameSuggestion, findActiveRequest, getCreatorApplicationForUser, getStreamerProfileById, getStreamerProfileBySlug, listAdminCreatorProfiles, listCreatorApplicationChecks, listCreatorApplicationEvents, listCreatorApplicationsForAdmin, listApprovedCatalog, listCreatorRequests, listGameSuggestions, listPublicRequests, listUsersForAdminOnboarding, reviewCreatorApplication, setCatalogOwnership, setCreatorApproval, updateCreatorApplicationCheck, updateGameSuggestionStatus, updateRequestStatus, updateStreamerProfile } from "./db";
...
export const appRouter = t.router({
  ...
  booking: t.router({
    ...
    classifyRequest: creatorProcedure.input(z.object({ id: z.number().int().positive(), streamerProfileId: z.number().int().positive(), status: requestStatus })).mutation(({ ctx, input }) => { if (ctx.user?.role !== "admin" && ctx.user?.streamerProfileId !== input.streamerProfileId) throw new TRPCError({ code: "FORBIDDEN" }); return updateRequestStatus(input); }),
    setOwnership: creatorProcedure.input(z.object({ id: z.number().int().positive(), streamerProfileId: z.number().int().positive(), ownershipStatus: z.enum(["unconfirmed", "owned"]) })).mutation(({ ctx, input }) => { if (ctx.user?.role !== "admin" && ctx.user?.streamerProfileId !== input.streamerProfileId) throw new TRPCError({ code: "FORBIDDEN" }); return setCatalogOwnership(input); }),
    ...
    myProfile: creatorProcedure.query(({ ctx }) => ctx.user?.streamerProfileId ? getStreamerProfileById(ctx.user.streamerProfileId) : undefined),
    updateProfile: creatorProcedure.input(z.object({ id: z.number().int().positive(), displayName: z.string().trim().min(2).max(160), bio: z.string().trim().max(2000).optional(), gamerTags: z.string().max(2000).optional(), streamLinks: z.string().max(2000).optional() })).mutation(({ ctx, input }) => { if (ctx.user?.role !== "admin" && ctx.user?.streamerProfileId !== input.id) throw new TRPCError({ code: "FORBIDDEN" }); return updateStreamerProfile(input); }),
